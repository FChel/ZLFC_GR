  ...
  return BaseController.extend("au.gov.defence.roman.zfss.gr.controller.PO", {
      ...
      oModel.create("/PurchaseOrders", oData, {
        success: function (oResult, oResponse) {
          ...
          if (bError) {
            ...
            var oRating = oController.getModel("searchHelp").createEntry("/FeedbackRatings").getObject();
            oRating.Rating = 0;
            oRating.Comments = "";
                ...
                beginButton: new sap.m.Button({
                  ...
                  press: function (oEvent1) {
                    ...
                    if(!oRating.Rating && !oRating.Comments){
                      ...
                      oRating.Rating += "";
                      oController.getModel("searchHelp").create("/FeedbackRatings", oRating, {
                        success: function(){
                          ratingPromise.resolve();
                        },
                        error: function(){
    ...
    _onNavBack: function (oEvent) {
      ...
      MessageBox.warning(this.getResourceBundle().getText("confirmExit"), {
        ...
        onClose: function(sAction) {
          if (sAction === MessageBox.Action.OK) {
            if (oController.getModel("viewModel").getProperty("/isVim") === true) {
              history.go(-1);
            } else {
    ...
    onPOValueHelpRequested: function(oEvent) {
      ...
      if (! this.oValueHelpDialog ) {
        ...
        this.oValueHelpDialog.getTableAsync().then(function (oTable) {
          oTable.setModel(this.getOwnerComponent().getModel("searchHelp"));
          oTable.setModel(oColModel, "columns");
          if (oTable.bindRows) {
            oTable.bindRows("/PurchaseOrders");
          }
